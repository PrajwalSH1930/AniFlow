...
export default function AnimeDetails() {
  ...
  const [activeTab, setActiveTab] = useState('overview'); // 'overview' | 'episodes' | 'trailer'
  const [selectedRange, setSelectedRange] = useState(0);

  const { isInWatchlist, getWatchStatus, addToWatchlist, removeFromWatchlist } = useWatchlist();
  ...
  useEffect(() => {
    ...
    async function loadDetails() {
      try {
        ...
        // Fetch anime details and all available episodes concurrently
        const [detailsData, episodesData] = await Promise.all([
          animeService.getAnimeDetails(id),
          animeService.getAnimeEpisodes(id, 300), // Fetches up to 300 episodes via pagination loop
        ]);
  ...
  // Split episodes into blocks for large anime
    ...
    <div className="min-h-screen pb-20">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 -mt-32 sm:-mt-44 relative z-10">
        <div className="flex flex-col md:flex-row gap-8 items-start">
          ...
          {/* Left Column: Poster Card & Watchlist Actions */}
          ...
          <div className="flex-1 space-y-6 w-full">
            ...
            {activeTab === 'episodes' && (
              <div className="space-y-4">
                {/* Range Filter for Series with >50 Episodes */}
                {totalChunks > 1 && (
                ...
                {visibleEpisodes.length === 0 ? (
                  <p className="text-sm text-slate-500 py-6">No episode listings available.</p>
                ) : (
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-center gap-3 overflow-hidden">
                            <span className="w-8 h-8 rounded-lg bg-slate-800 text-brand-primary font-bold text-xs flex items-center justify-center flex-shrink-0">
                              {epNum}
                            </span>
